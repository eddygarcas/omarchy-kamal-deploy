...
# tail of Kamal's own `kamal init` template (aliases/ssh/volumes/asset_path/
# boot/accessories guidance) for a brand-new base config — into
# config/deploy.yml or config/deploy.<env>.yml.
#
# A named environment with no base config yet gets both: the base is
# meaningless to skip (an override with nothing to override), and it saves
# a second trip through the wizard.
set -uo pipefail
...
base_file="$config_dir/deploy.yml"

if [[ -z "$ENV_NAME" ]]; then
  out_file="$base_file"
  out_include_header=true
else
  out_file="$config_dir/deploy.$ENV_NAME.yml"
  out_include_header=false
fi
...
if [[ -z "$ENV_NAME" && -f "$base_file" ]]; then
